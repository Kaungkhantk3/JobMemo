...
export type GmailSuggestionReviewFormState = {
  company: string;
  position: string;
  notes: string;
  hideEmail: boolean;
...
export function GmailSuggestionReviewModal({
  open,
  suggestion,
  submitting = false,
  onClose,
  ...
  open: boolean;
  suggestion: GmailSuggestion | null;
  submitting?: boolean;
  onClose: () => void;
  onConfirm: (payload: GmailSuggestionReviewFormState) => void;
}) {
  const [form, setForm] = useState<GmailSuggestionReviewFormState>(() => ({
    company: suggestion?.company ?? "",
    position: suggestion?.position ?? "",
    notes: suggestion?.notes ?? "",
    hideEmail: !!suggestion?.hidden,
  }));
      ...
      <div className="w-full max-w-xl rounded-3xl border border-zinc-200 bg-white shadow-2xl">
        <div className="border-b border-zinc-200 px-5 py-5 md:px-6">
          ...
          <p className="mt-1 text-[13px] leading-6 text-zinc-600">
            Review the company, role, and notes before JobMemo creates or
            updates the tracked application.
          </p>
        </div>
        ...
        <div className="space-y-4 px-5 py-5 md:px-6">
          <div className="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3">
            ...
            <p className="mt-1 text-[12px] text-zinc-500">
              Last updated {suggestion.syncedAt ?? "just now"}
            </p>
          </div>
          ...
          <label className="space-y-1.5">
            <span className="text-[12px] font-medium text-zinc-600">Notes</span>
            <textarea
          ...
          <label className="flex items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3">
            ...
            <span className="text-[13px] text-zinc-700">
              Hide this suggestion after importing
            </span>
          </label>
        </div>

        <div className="flex items-center justify-end gap-2 border-t border-zinc-200 px-5 py-4 md:px-6">
          <button
            type="button"
            onClick={onClose}
